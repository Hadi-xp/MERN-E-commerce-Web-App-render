...
paymentRouter.get('/callback',async(req,res)=>{
    try {
        if(req.query.Status && req.query.Status !== 'OK'){
            ...
            await order.save();
            await payment.save();
            return res.redirect(`https://hadi-xp.github.io/mern-render/order-confirmation/${order._id}`);
        }
        let payment = await Payment.findOne({resNumber:req.query.Authority});
        ...
        if(response.data.data.code === 100){
            ...
            await order.save();
            await payment.save();
            res.redirect(`https://hadi-xp.github.io/mern-render/order-confirmation/${order._id}`);
        }
    } catch (error) {
        console.log(error);
